...
class ColumbusMlTapDetector(
    ...
    companion object {
        ...
        private const val MIN_Z_DOMINANCE_RATIO = 0.85f // Back tap requires Z-axis impulse to represent >= 85% of total vector energy
        private const val SCREEN_TOUCH_LOCKOUT_MS = 400L // Ignore back taps within 400ms of a screen touch
        ...
        when (event.sensor.type) {
            ...
            Sensor.TYPE_ACCELEROMETER -> {
                ...
                if (confidence >= CONFIDENCE_THRESHOLD) {
                    lastTapTime = now
                    Log.i(TAG, "BACK_TAP_ACCEPTED: Dominant Z-axis impulse (zRatio=%.2f >= 0.85) | Conf: %.1f%%".format(zDominanceRatio, confidence * 100f))
    ...
    private fun runColumbusInference(
        ...
        if (zDominanceRatio < MIN_Z_DOMINANCE_RATIO) {
            return Pair(0.12f, "Screen Tap X/Y Shear (zRatio %.2f < 0.85)".format(zDominanceRatio))
        }
